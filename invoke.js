...
async function invokechaincode(USER_NAME, CHANNEL_NAME, CHAIN_CODE_NAME, CHAIN_CODE_FUNCTION_NAME, ...ARGS) {
    let return_value;
    try {
        ...
        if (!userExists) {
            ...
            return_value = 'An identity for the user "user1" does not exist in the wallet. Run the registerUser.js application before retrying';
        }
        else {
            ...
            return_value = await contract.submitTransaction(CHAIN_CODE_FUNCTION_NAME, ...ARGS);
